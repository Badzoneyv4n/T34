...
class PocketOption:
    ...
    def __init__(self, ssid, demo):
        ...
        global_value.logger("Mode Demo: %s" % str(demo), "INFO")
        self.suspend = 0.5
        self.thread = None
    ...
    #     await self.api.buyv3(amount, active, action, expirations, req_id)
    ...
    def check_win(self, id_number=None, expiration_sec=300, buffer_sec=30):
        """
        Waits for the trade to close and returns the profit and result.
        - expiration_sec: The trade's expiration in seconds (e.g., 300 for 5M)
        - buffer_sec: Extra buffer to wait for the result to settle
        """
        if not id_number:
            order_info = self.get_async_order()
            return order_info

        max_wait = expiration_sec + buffer_sec
        start_t = time.time()
        order_info = None
        ...
        while True:
            try:
                ...
                # Silently ignore while waiting, no spam
                pass

            elapsed = time.time() - start_t
            if elapsed >= max_wait:
                global_value.logger(
                    f"Timeout: Order result not available after {max_wait}s.", "ERROR"
                )
                return None, "unknown"

            if int(elapsed) % 30 == 0:
                global_value.logger(
                    f"Waiting for order result... {int(elapsed)}s elapsed.", "INFO"
                )
        ...
        if order_info and "profit" in order_info:
            ...
            global_value.logger("Invalid order info received.", "ERROR")
            return None, "unknown"
